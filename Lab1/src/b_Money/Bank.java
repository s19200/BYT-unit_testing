...
public class Bank {
	...
	public Hashtable getAccountList (){
		return this.accountlist;
	}
	//method to get account from account list
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			accountlist.put(accountid, new Account(accountid, this.currency));			//change get(accountid) to put(accountid, accountlist.get(accountid) because we insert an element into hashtable
		}
	}
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {		//added negation since we throw exception only if account does NOT exist
	...
	public Integer getBalance(String accountid) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			return account.getBalance().getAmount().intValue();		//added .intValue to convert Double(which is the return value of getAmount) to Integer
		}
	}
	...
	public void transfer(String fromaccount, Bank tobank, String toaccount, Money amount) throws AccountDoesNotExistException {
		if (!this.accountlist.containsKey(fromaccount) || !tobank.accountlist.containsKey(toaccount)) {
			...
			this.accountlist.get(fromaccount).withdraw(amount);
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		if (!this.accountlist.containsKey(fromaccount) || !this.accountlist.containsKey(toaccount)) {		//added code responsible for throwing the exception
			throw new AccountDoesNotExistException();
		}
		transfer(fromaccount, this, toaccount, amount);		//changed third argument form fromaccount to toaccount
	}
